...
for event in pygame.event.get():
    ...
    if event.type == KEYDOWN:
        # move user car to the left if in the right lane
        if event.key in [K_a, K_LEFT] and car_loc.centerx > left_lane:
            car_loc = car_loc.move([-int(road_w/2), 0])
        # move user car to the right if in the left lane
        if event.key in [K_d, K_RIGHT] and car_loc.centerx < right_lane:
            car_loc = car_loc.move([int(road_w/2), 0])
